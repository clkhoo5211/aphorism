...
export const DivinationLotCard: React.FC<DivinationLotCardProps> = ({ lot }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-6 border-b border-cyan-500/10 flex justify-between items-center bg-slate-950/50">
        <div>
          <span className="text-[10px] font-mono text-cyan-500 uppercase tracking-[0.2em]">Oracle Response Node #{lot.id < 10 ? `0${lot.id}` : lot.id}</span>
        ...
        <div className={`px-4 py-1 rounded-full border ${getFortuneColor(lot.fortune)} text-xs font-bold uppercase tracking-widest`}>
          {lot.fortune}
        </div>
      </div>
        ...
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div className="space-y-2">
            {lot.poem.map((line, idx) => (
              <p key={idx} className="text-xl md:text-2xl font-serif text-slate-100 tracking-widest leading-relaxed">
                {line}
              </p>
            ))}
          </div>
